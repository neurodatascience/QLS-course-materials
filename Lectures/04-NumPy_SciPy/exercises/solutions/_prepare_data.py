...
def make_difumo():
    ...
    noise = rng.normal(
        scale=difumo_data.max() / 16, size=difumo_data.shape
    ).astype("float32")
    difumo_mask = image.get_data(
        image.resample_to_img(
            datasets.load_mni152_brain_mask(2), difumo, interpolation="nearest"
        )
    )
...
difumo_misaligned_data = image.get_data(difumo_misaligned)[
    ..., z_slice // 2, :
]
