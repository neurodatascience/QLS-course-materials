...
\label{sec:orge2e1fcf}
\subsection{Recap of part 1}
\label{sec:org937438e}
\begin{frame}[label={sec:org602e656}]{Recap of part 1}
...
\begin{frame}[label={sec:org39e4b38}]{Recap of part 1}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org7f3dcde}]{Notation \& vocabulary}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:orgd9c4748}]{How to set parameters: Empirical Risk Minimization}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgd862ab0}]{How to set parameters: Empirical Risk Minimization}
...
\begin{frame}[label={sec:orgf0a8e1c}]{Evaluating a model}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:orgd05af92}]{Training error is a biased estimator of the risk}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgf8f2636}]{Training error is a biased estimator of the risk}
\begin{itemize}
\item 30 different models (eg 30 possible values for \(\hat{\beta}_0, \hat{\bbeta}\))
\item All have a risk (expected accuracy) of 0.5
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orga2dcb57}]{Training error is a biased estimator of the risk}
...
\begin{frame}[label={sec:org2d73453}]{Training error is a biased estimator of the risk}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org0b5b8b3}]{Training error is a biased estimator of the risk}
...
\begin{frame}[label={sec:org4ffc681}]{Training error is a biased estimator of the risk}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgaec64d4}]{Training error is a biased estimator of the risk}
\begin{itemize}
...
\begin{frame}[label={sec:org252cba5}]{Estimating prediction performance}
...
\label{sec:org17cbea6}
\begin{frame}[label={sec:org5598213},fragile]{scikit-learn "estimator API": \texttt{fit; predict}}
 \begin{minted}[]{python}
estimator = Ridge()
...
\begin{frame}[label={sec:orgb61ff73},fragile]{Evaluating performance with \texttt{sklearn.metrics}}
 \begin{minted}[]{python}
estimator = Ridge()
estimator.fit(X_train, y_train)
predictions = estimator.predict(X_test)
...
\begin{frame}[label={sec:org608b829},fragile]{Some possible metrics for regression}
...
\subsection{cv}
\label{sec:org03cd05c}
\begin{frame}[label={sec:org95c7b16},fragile]{Cross-validation}
 \begin{center}
...
\label{sec:orgb40df16}
\subsection{nested cv}
\label{sec:org11bcbf6}
\begin{frame}[label={sec:org3be39b9}]{Need for regularization}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org846987c}]{Regularization}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org659d2ba}]{}
\begin{columns}
...
\begin{frame}[label={sec:orgfbf9ebc}]{Setting hyperparameters}
...
\end{frame}
\begin{frame}[label={sec:org6916c68}]{Nested cross-validation}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgea5866e}]{One split}
\begin{center}
\includegraphics[width=.9\linewidth]{figures/generated/train_eval_test/datasets.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:org4bb6cf6},fragile]{Nested cross-validation}
 \begin{center}
...
\begin{frame}[label={sec:org9416c25},fragile]{Nested cross-validation with scikit-learn}
 \begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:orgd017323},fragile]{Implementing nested CV}
...
\label{sec:org9fca622}
\subsection{Intro}
\label{sec:org7d9cb21}
\begin{frame}[label={sec:orga27c055}]{Dimensionality reduction}
Linear regression: projection on the column space of \(X\)
\vspace{10pt}
\begin{structureenv} %% top
\begin{columns}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:orga3fccb5}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org6818fd7},fragile]{Simulated data for linear regression}
 \begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org2e74ec6}]{Model complexity: overfitting}
\begin{itemize}
...
\label{sec:org37ea4e2}
\begin{frame}[label={sec:org9f7dff2}]{Univariate feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:org739e07b}]{Original regression problem}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org92f17b9}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:org4e31563}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:org5bc523a}]{Univariate feature selection}
...
\label{sec:orgc773a89}
\begin{frame}[label={sec:org25b10f0}]{Dataset transformations}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org0449169},fragile]{scikit-learn "transformer API": \texttt{fit; transform}}
 \begin{minted}[]{python}
...
\begin{frame}[label={sec:org6f26cfe},fragile]{\texttt{feature\_selection.SelectKBest}}
...
\begin{frame}[label={sec:org8e293cd},fragile]{Fit the transformer only on train data!}
 \begin{minted}[]{python}
transformer = SelectKBest()
transformed_train = transformer.fit_transform(X_train)
...
\begin{frame}[label={sec:orgd1db3b7},fragile]{Pipelines}
...
\label{sec:org4c41bfe}
\begin{frame}[label={sec:org448c649}]{Linear decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org1d81d0a}]{Linear regression: projection on the column space of \(X\)}
\begin{structureenv} %% top
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org1aa0e7a}]{Linear decomposition: low-rank approximation of \(\X\)}
Minimize
\begin{equation}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org5f88526}]{Linear regression after dimensionality reduction}
\begin{equation}
\hat{\y} = \W \, \hat{\bbeta}
\end{equation}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgd46f6ca}]{Prediction for a new data point \(\x \in \R^{p}\)}
\begin{itemize}
...
\x \in \R^p \rightarrow \text{projection} \rightarrow \mathbold{w} \in \R^k \rightarrow \langle \cdot \, , \, \hat{\bbeta}\rangle \rightarrow \hat{y} \in \R
    \end{equation}
\end{frame}
\begin{frame}[label={sec:org89d5b3d}]{Principal Component Analysis}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org5e4390b}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org45dcf2d}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orgc31b77a}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org27d7fc4}]{Other decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org33fa440}]{Ridge regression and PCA}
\begin{itemize}
...
\label{sec:org5d520fb}
\begin{frame}[label={sec:orge48c810}]{(Cross-)validation experiments are simulations}
...
\end{frame}
\begin{frame}[label={sec:org723bfce}]{(Cross-)validation experiments are simulations}
The validation experiments must simulate what will happen when deploying the trained model in production -- when starting to use it in real life.
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org7b35c29}]{Split choice example: time series}
...
\end{frame}
\begin{frame}[label={sec:orgcf39d1e}]{Remember that CV training sets overlap}
\begin{center}
...
\begin{frame}[label={sec:org0689899},fragile]{Some pitfalls with cross-validation}
 \small
