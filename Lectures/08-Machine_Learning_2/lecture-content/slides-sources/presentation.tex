...
\label{sec:orgc313a11}
\subsection{Recap of part 1}
\label{sec:orgc293ca5}
\begin{frame}[label={sec:orga096960}]{Recap of part 1}
...
\begin{frame}[label={sec:orge1e3d8f}]{Recap of part 1}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgeb6e934}]{Notation \& vocabulary}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org57d8936}]{How to set parameters: Empirical Risk Minimization}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orge967d7f}]{How to set parameters: Empirical Risk Minimization}
...
\begin{frame}[label={sec:org5e6cf29}]{Evaluating a model}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org9b965b9}]{Training error is a biased estimator of the risk}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgf97efbd}]{Training error is a biased estimator of the risk}
\begin{itemize}
\item 30 different models (eg 30 possible values for \(\hat{\beta}_0, \hat{\bbeta}\))
\item All have a risk (expected accuracy) of 0.5
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org4a05f42}]{Training error is a biased estimator of the risk}
...
\begin{frame}[label={sec:org5fa7ed3}]{Training error is a biased estimator of the risk}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org1a8fc48}]{Training error is a biased estimator of the risk}
...
\begin{frame}[label={sec:orga15baf5}]{Training error is a biased estimator of the risk}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org0f71143}]{Training error is a biased estimator of the risk}
\begin{itemize}
...
\begin{frame}[label={sec:org58bb7c6}]{Estimating prediction performance}
...
\label{sec:org8a27de8}
\begin{frame}[label={sec:org9518b92},fragile]{scikit-learn "estimator API": \texttt{fit; predict}}
 \begin{minted}[]{python}
estimator = Ridge()
...
\begin{frame}[label={sec:org42e965b},fragile]{Evaluating performance with \texttt{sklearn.metrics}}
 \begin{minted}[]{python}
estimator = Ridge()
estimator.fit(X_train, y_train)
predictions = estimator.predict(X_test)
...
\begin{frame}[label={sec:orgd4f630a},fragile]{Some possible metrics for regression}
...
\subsection{cv}
\label{sec:orgd8609e9}
\begin{frame}[label={sec:orgcb40ea2},fragile]{Cross-validation}
 \begin{center}
...
\label{sec:org2dd157f}
\subsection{nested cv}
\label{sec:org2afa49e}
\begin{frame}[label={sec:org4488dbf}]{Need for regularization}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org4a49c6c}]{Regularization}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org27ed663}]{}
\begin{columns}
...
\begin{frame}[label={sec:orged6026d}]{Setting hyperparameters}
...
\end{frame}
\begin{frame}[label={sec:org849fe35}]{Nested cross-validation}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org62132cf}]{One split}
\begin{center}
\includegraphics[width=.9\linewidth]{figures/generated/train_eval_test/datasets.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:org15724d4},fragile]{Nested cross-validation}
 \begin{center}
...
\begin{frame}[label={sec:orgcf5e0b3},fragile]{Nested cross-validation with scikit-learn}
 \begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:orgbf0990a},fragile]{Implementing nested CV}
...
\label{sec:org7a240aa}
\subsection{Intro}
\label{sec:org783444a}
\begin{frame}[label={sec:org54e98e6}]{Dimensionality reduction}
Linear regression: projection on the column space of \(X\)
\vspace{10pt}
\begin{structureenv} %% top
\begin{columns}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:orgb490ef4}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org0e530c7},fragile]{Simulated data for linear regression}
 \begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org4dba656}]{Model complexity: overfitting}
\begin{itemize}
...
\label{sec:org2753038}
\begin{frame}[label={sec:org4cee03a}]{Univariate feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:org21e6f45}]{Original regression problem}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org3011741}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:org95ecdd5}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:orgd4dfa10}]{Univariate feature selection}
...
\label{sec:org02a7f28}
\begin{frame}[label={sec:orgff8897e}]{Dataset transformations}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgaa15691},fragile]{scikit-learn "transformer API": \texttt{fit; transform}}
 \begin{minted}[]{python}
...
\begin{frame}[label={sec:orga51836e},fragile]{\texttt{feature\_selection.SelectKBest}}
...
\begin{frame}[label={sec:org06f669a},fragile]{Fit the transformer only on train data!}
 \begin{minted}[]{python}
transformer = SelectKBest()
transformed_train = transformer.fit_transform(X_train)
...
\begin{frame}[label={sec:org3bcc1c8},fragile]{Pipelines}
...
\label{sec:orgcd0f3c1}
\begin{frame}[label={sec:orgf7d2fa7}]{Linear decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgdbcd392}]{Linear regression: projection on the column space of \(X\)}
\begin{structureenv} %% top
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org1f766df}]{Linear decomposition: low-rank approximation of \(\X\)}
Minimize
\begin{equation}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org91c6f18}]{Linear regression after dimensionality reduction}
\begin{equation}
\hat{\y} = \W \, \hat{\bbeta}
\end{equation}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgd68f4e5}]{Prediction for a new data point \(\x \in \R^{p}\)}
\begin{itemize}
...
\x \in \R^p \rightarrow \text{projection} \rightarrow \mathbold{w} \in \R^k \rightarrow \langle \cdot \, , \, \hat{\bbeta}\rangle \rightarrow \hat{y} \in \R
    \end{equation}
\end{frame}
\begin{frame}[label={sec:org352a5ad}]{Principal Component Analysis}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org37fcd75}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org786c3de}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org269827d}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orgdaec2cb}]{Other decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgc00e416}]{Ridge regression and PCA}
\begin{itemize}
...
\label{sec:org9ae9869}
\begin{frame}[label={sec:orgb316f86}]{(Cross-)validation experiments are simulations}
...
\end{frame}
\begin{frame}[label={sec:orgfa29489}]{(Cross-)validation experiments are simulations}
The validation experiments must simulate what will happen when deploying the trained model in production -- when starting to use it in real life.
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org5f280d8}]{Split choice example: time series}
...
\end{frame}
\begin{frame}[label={sec:org96c1d51}]{Remember that CV training sets overlap}
\begin{center}
...
\begin{frame}[label={sec:orgf23f157},fragile]{Some pitfalls with cross-validation}
 \small
