...
\label{sec:orgbb865cc}
\subsection{Recap of part 1}
\label{sec:org9c6f7e6}
\begin{frame}[label={sec:org0fcfdab}]{Recap of part 1}
...
\begin{frame}[label={sec:org846e27a}]{Recap of part 1}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org44a7fb1}]{Notation \& vocabulary}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org342a56b}]{How to set parameters: Empirical Risk Minimization}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgc2a4937}]{How to set parameters: Empirical Risk Minimization}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org6104865}]{Need for fresh test data}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org3883244}]{Estimating prediction performance}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\label{sec:org3b3276a}
\begin{frame}[label={sec:org25bdc6b},fragile]{scikit-learn "estimator API": \texttt{fit; predict}}
 \begin{minted}[]{python}
estimator = Ridge()
...
\begin{frame}[label={sec:org843834c},fragile]{Evaluating performance with \texttt{sklearn.metrics}}
 \begin{minted}[]{python}
estimator = Ridge()
estimator.fit(X_train, y_train)
predictions = estimator.predict(X_test)
...
\subsection{cv}
\label{sec:orga888d37}
\begin{frame}[label={sec:orgea3259f},fragile]{Cross-validation}
 \begin{center}
...
\label{sec:org9962cc7}
\subsection{nested cv}
\label{sec:org6d9493f}

\begin{frame}[label={sec:org80bc390}]{Need for regularization}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org99893bd}]{Regularization}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org35e7111}]{}
\begin{columns}
...
\begin{frame}[label={sec:orgb4b9d6f}]{Setting hyperparameters}
...
\end{frame}
\begin{frame}[label={sec:org53c1069}]{Nested cross-validation}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org16aa3a1}]{One split}
\begin{center}
\includegraphics[width=.9\linewidth]{figures/generated/train_eval_test/datasets.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:orgdc5f920},fragile]{Nested cross-validation}
 \begin{center}
...
\begin{frame}[label={sec:orgd52c292},fragile]{Nested cross-validation with scikit-learn}
 \begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:org9db64e4},fragile]{Implementing nested CV}
 \texttt{ex\_04\_nested\_cross\_validation\_questions.py}
\end{frame}
\section{Dimensionality reduction}
\label{sec:orgcf1f20a}
\subsection{Intro}
\label{sec:org0a77b8f}
\begin{frame}[label={sec:orgbe1b1c5}]{Dimensionality reduction}
\begin{equation}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org6a5235f}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgc8e4129},fragile]{Simulated data for linear regression}
 \begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgff0a592}]{Model complexity: overfitting}
\begin{itemize}
...
\label{sec:org161b3c2}
\begin{frame}[label={sec:orgd070369}]{Univariate feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:orge4c816a}]{Univariate feature selection}
...
\label{sec:orgdb4efd0}
\begin{frame}[label={sec:orga4480d9}]{Dataset transformations}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org42f1c17},fragile]{scikit-learn "transformer API": \texttt{fit; transform}}
 \begin{minted}[]{python}
...
\begin{frame}[label={sec:orgd4dc3a3},fragile]{\texttt{feature\_selection.SelectKBest}}
...
\begin{frame}[label={sec:org2824da7},fragile]{Fit the transformer only on train data!}
 \begin{minted}[]{python}
transformer = SelectKBest()
transformed_train = transformer.fit_transform(X_train)
...
\begin{frame}[label={sec:org1c7b402},fragile]{Pipelines}
...
\label{sec:org908292f}
\begin{frame}[label={sec:org87e9cde}]{Linear decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org778e6c0}]{Linear regression: projection on the column space of \(X\)}
\begin{structureenv} %% top
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org8825df6}]{Linear decomposition: low-rank approximation of \(\X\)}
Minimize
\begin{equation}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org47f9ffb}]{Linear regression after dimensionality reduction}
\begin{equation}
\hat{\y} = \W \, \hat{\bbeta}
\end{equation}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org2087abe}]{Prediction for a new data point \(\x \in \R^{p}\)}
\begin{itemize}
...
\x \in \R^p \rightarrow \text{projection} \rightarrow \mathbold{w} \in \R^k \rightarrow \langle \cdot \, , \, \hat{\bbeta}\rangle \rightarrow \hat{y} \in \R
    \end{equation}
\end{frame}
\begin{frame}[label={sec:org8a35dfc}]{Principal Component Analysis}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgd1e10d4}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orgdbdba16}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org7cff9b9}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org9f11276}]{Other decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org41afd6a}]{Ridge regression and PCA}
\begin{itemize}
...
\label{sec:orgfe91e65}
\begin{frame}[label={sec:org3ed1c2f}]{Split choice example: time series}
...
\end{frame}
\begin{frame}[label={sec:orgafa00ce}]{Remember that CV training sets overlap}
\begin{center}
...
\begin{frame}[label={sec:org73c0fdf},fragile]{Some pitfalls with cross-validation}
 \small
