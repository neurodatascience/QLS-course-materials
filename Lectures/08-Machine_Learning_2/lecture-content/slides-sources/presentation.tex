...
\label{sec:org83d7348}
\subsection{Recap of part 1}
\label{sec:org9660c30}
\begin{frame}[label={sec:org454f714}]{Recap of part 1}
...
\begin{frame}[label={sec:org8ae6627}]{Recap of part 1}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgf330366}]{Notation \& vocabulary}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:orgd248ac0}]{How to set parameters: Empirical Risk Minimization}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org5c1f689}]{How to set parameters: Empirical Risk Minimization}
...
\begin{frame}[label={sec:org046fcdc}]{Evaluating a model}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org4111dd5}]{Training error is a biased estimator of the risk}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org61f8293}]{Training error is a biased estimator of the risk}
\begin{itemize}
\item 30 different models (eg 30 possible values for \(\hat{\beta}_0, \hat{\bbeta}\))
\item All have a risk (expected accuracy) of 0.5
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgdf455d8}]{Training error is a biased estimator of the risk}
...
\begin{frame}[label={sec:org0d6e3fe}]{Training error is a biased estimator of the risk}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgfbfc577}]{Training error is a biased estimator of the risk}
...
\begin{frame}[label={sec:org4e0a088}]{Training error is a biased estimator of the risk}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org3728f96}]{Training error is a biased estimator of the risk}
\begin{itemize}
...
\begin{frame}[label={sec:org0ec87a9}]{Estimating prediction performance}
...
\label{sec:org576cc22}
\begin{frame}[label={sec:orgf38a0fa},fragile]{scikit-learn "estimator API": \texttt{fit; predict}}
 \begin{minted}[]{python}
estimator = Ridge()
...
\begin{frame}[label={sec:org7928dab},fragile]{Evaluating performance with \texttt{sklearn.metrics}}
 \begin{minted}[]{python}
estimator = Ridge()
estimator.fit(X_train, y_train)
predictions = estimator.predict(X_test)
...
\begin{frame}[label={sec:org02ab7d9},fragile]{Some possible metrics for regression}
...
\subsection{cv}
\label{sec:orgab64737}
\begin{frame}[label={sec:orgd387584},fragile]{Cross-validation}
 \begin{center}
...
\label{sec:org0f6a9ff}
\subsection{nested cv}
\label{sec:orgd077dd1}
\begin{frame}[label={sec:org2f85e78}]{Need for regularization}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:orgf95ca6c}]{Regularization}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org93a620b}]{}
\begin{columns}
...
\begin{frame}[label={sec:org2498cd3}]{Setting hyperparameters}
...
\end{frame}
\begin{frame}[label={sec:orga48b726}]{Nested cross-validation}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org471d7bb}]{One split}
\begin{center}
\includegraphics[width=.9\linewidth]{figures/generated/train_eval_test/datasets.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:orgcee1ecd},fragile]{Nested cross-validation}
 \begin{center}
...
\begin{frame}[label={sec:orgafc5823},fragile]{Nested cross-validation with scikit-learn}
 \begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:org9ff10eb},fragile]{Implementing nested CV}
...
\label{sec:org77e2aee}
\subsection{Intro}
\label{sec:orgd395339}
\begin{frame}[label={sec:org1de7a38}]{Dimensionality reduction}
Linear regression: projection on the column space of \(X\)
\vspace{10pt}
\begin{structureenv} %% top
\begin{columns}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org3880a71}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org9dc77c5},fragile]{Simulated data for linear regression}
 \begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org2dc4712}]{Model complexity: overfitting}
\begin{itemize}
...
\label{sec:orgd01e64b}
\begin{frame}[label={sec:org74259a6}]{Univariate feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:org423efeb}]{Original regression problem}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:orgad7f04f}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:org036a1db}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:orgd2f0189}]{Univariate feature selection}
...
\label{sec:org7aaecf6}
\begin{frame}[label={sec:org9febf11}]{Dataset transformations}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org271f831},fragile]{scikit-learn "transformer API": \texttt{fit; transform}}
 \begin{minted}[]{python}
...
\begin{frame}[label={sec:orgaec5042},fragile]{\texttt{feature\_selection.SelectKBest}}
...
\begin{frame}[label={sec:org597a965},fragile]{Fit the transformer only on train data!}
 \begin{minted}[]{python}
transformer = SelectKBest()
transformed_train = transformer.fit_transform(X_train)
...
\begin{frame}[label={sec:org2451641},fragile]{Pipelines}
...
\label{sec:org619b362}
\begin{frame}[label={sec:org225ebb1}]{Linear decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org2fae070}]{Linear regression: projection on the column space of \(X\)}
\begin{structureenv} %% top
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:orgad7c7c4}]{Linear decomposition: low-rank approximation of \(\X\)}
Minimize
\begin{equation}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgc4d0877}]{Linear regression after dimensionality reduction}
\begin{equation}
\hat{\y} = \W \, \hat{\bbeta}
\end{equation}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org0e676e9}]{Prediction for a new data point \(\x \in \R^{p}\)}
\begin{itemize}
...
\x \in \R^p \rightarrow \text{projection} \rightarrow \mathbold{w} \in \R^k \rightarrow \langle \cdot \, , \, \hat{\bbeta}\rangle \rightarrow \hat{y} \in \R
    \end{equation}
\end{frame}
\begin{frame}[label={sec:org32eaf4e}]{Principal Component Analysis}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org792a5bd}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org609afff}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orgbaa40ab}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org9f6b59c}]{Other decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgd722649}]{Ridge regression and PCA}
\begin{itemize}
...
\label{sec:org5bd23a8}
\begin{frame}[label={sec:org34c9966}]{(Cross-)validation experiments are simulations}
...
\end{frame}
\begin{frame}[label={sec:orgc408b94}]{(Cross-)validation experiments are simulations}
The validation experiments must simulate what will happen when deploying the trained model in production -- when starting to use it in real life.
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org1d67af4}]{Split choice example: time series}
...
\end{frame}
\begin{frame}[label={sec:orgda7cc52}]{Remember that CV training sets overlap}
\begin{center}
...
\begin{frame}[label={sec:org32d7060},fragile]{Some pitfalls with cross-validation}
 \small
