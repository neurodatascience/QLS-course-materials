...
\label{sec:orgb4fdd5e}
\subsection{Recap of part 1}
\label{sec:orgcc479e9}
\begin{frame}[label={sec:org17ce4b2}]{Recap of part 1}
...
\begin{frame}[label={sec:org1cda4e0}]{Recap of part 1}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgb69a2a5}]{Notation \& vocabulary}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org39764f8}]{How to set parameters: Empirical Risk Minimization}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orged7d0b4}]{How to set parameters: Empirical Risk Minimization}
...
\begin{frame}[label={sec:org3f3c9ad}]{Evaluating a model}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org4a98733}]{Training error is a biased estimator of the risk}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org1ffad15}]{Training error is a biased estimator of the risk}
\begin{itemize}
\item 30 different models (eg 30 possible values for \(\hat{\beta}_0, \hat{\bbeta}\))
\item All have a risk (expected accuracy) of 0.5
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orga9f9294}]{Training error is a biased estimator of the risk}
...
\begin{frame}[label={sec:org7a3f1e8}]{Training error is a biased estimator of the risk}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgbeaf385}]{Training error is a biased estimator of the risk}
...
\begin{frame}[label={sec:org2f1873d}]{Training error is a biased estimator of the risk}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org51f45a3}]{Training error is a biased estimator of the risk}
\begin{itemize}
...
\begin{frame}[label={sec:org43f4eae}]{Estimating prediction performance}
...
\label{sec:org645bd84}
\begin{frame}[label={sec:orgf5886c6},fragile]{scikit-learn "estimator API": \texttt{fit; predict}}
 \begin{minted}[]{python}
estimator = Ridge()
...
\end{frame}
\begin{frame}[label={sec:org2ff6b51},fragile]{Evaluating performance with \texttt{sklearn.metrics}}
 \begin{minted}[]{python}
estimator = Ridge()
estimator.fit(X_train, y_train)
predictions = estimator.predict(X_test)
...
\begin{frame}[label={sec:org0df69ed},fragile]{Some possible metrics for regression}
...
\subsection{cv}
\label{sec:org9055760}
\begin{frame}[label={sec:orgff101aa},fragile]{Cross-validation}
 \begin{center}
...
\label{sec:orgcc8a22d}
\subsection{nested cv}
\label{sec:org02db50c}
\begin{frame}[label={sec:org3c56779}]{Need for regularization}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org54dae17}]{Regularization}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:orgdd37f95}]{}
\begin{columns}
...
\begin{frame}[label={sec:org0dc4027}]{Setting hyperparameters}
...
\end{frame}
\begin{frame}[label={sec:orgd6b0338}]{Nested cross-validation}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgd9a3c63}]{One split}
\begin{center}
\includegraphics[width=.9\linewidth]{figures/generated/train_eval_test/datasets.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:org7194886},fragile]{Nested cross-validation}
 \begin{center}
...
\begin{frame}[label={sec:orga7eb04a},fragile]{Nested cross-validation with scikit-learn}
 \begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:orgeba6202},fragile]{Implementing nested CV}
...
\label{sec:orgc447bc6}
\subsection{Intro}
\label{sec:orgeec8e8e}
\begin{frame}[label={sec:org8005c91}]{Dimensionality reduction}
Linear regression: projection on the column space of \(X\)
\vspace{10pt}
\begin{structureenv} %% top
\begin{columns}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org7167d0b}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org3dbfbb2},fragile]{Simulated data for linear regression}
 \begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orge8444f9}]{Model complexity: overfitting}
\begin{itemize}
...
\label{sec:org06c3e5c}
\begin{frame}[label={sec:org2249ec3}]{Univariate feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:org3980a60}]{Original regression problem}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org2cda8a2}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:org95ad6cf}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:orga606b33}]{Univariate feature selection}
...
\label{sec:org5762c48}
\begin{frame}[label={sec:org7a37f62}]{Dataset transformations}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org8c6919f},fragile]{scikit-learn "transformer API": \texttt{fit; transform}}
 \begin{minted}[]{python}
...
\begin{frame}[label={sec:orgbfbe536},fragile]{\texttt{feature\_selection.SelectKBest}}
...
\begin{frame}[label={sec:org55f8f8e},fragile]{Fit the transformer only on train data!}
 \begin{minted}[]{python}
transformer = SelectKBest()
transformed_train = transformer.fit_transform(X_train)
...
\begin{frame}[label={sec:org8c9b8bd},fragile]{Pipelines}
...
\label{sec:org08728cd}
\begin{frame}[label={sec:org6337c9b}]{Linear decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org952db04}]{Linear regression: projection on the column space of \(X\)}
\begin{structureenv} %% top
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:orge692e78}]{Linear decomposition: low-rank approximation of \(\X\)}
Minimize
\begin{equation}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgf5b9cc9}]{Linear regression after dimensionality reduction}
\begin{equation}
\hat{\y} = \W \, \hat{\bbeta}
\end{equation}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgf7a0a48}]{Prediction for a new data point \(\x \in \R^{p}\)}
\begin{itemize}
...
\x \in \R^p \rightarrow \text{projection} \rightarrow \mathbold{w} \in \R^k \rightarrow \langle \cdot \, , \, \hat{\bbeta}\rangle \rightarrow \hat{y} \in \R
    \end{equation}
\end{frame}
\begin{frame}[label={sec:org49f6787}]{Principal Component Analysis}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org512f34e}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orgc2c439e}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orgc472c6f}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org944aa75}]{Other decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orge1b56ab}]{Ridge regression and PCA}
\begin{itemize}
...
\label{sec:org91cdec5}
\begin{frame}[label={sec:orga5a1502}]{(Cross-)validation experiments are simulations}
...
\end{frame}
\begin{frame}[label={sec:orga06eed6}]{(Cross-)validation experiments are simulations}
The validation experiments must simulate what will happen when deploying the trained model in production -- when starting to use it in real life.
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org7966e63}]{Split choice example: time series}
...
\end{frame}
\begin{frame}[label={sec:orgaede3a3}]{Remember that CV training sets overlap}
\begin{center}
...
\begin{frame}[label={sec:orga8e87f1},fragile]{Some pitfalls with cross-validation}
 \small
