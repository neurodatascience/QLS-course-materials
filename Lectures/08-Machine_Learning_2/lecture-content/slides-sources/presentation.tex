...
\label{sec:org7f7cf4a}
\subsection{Recap of part 1}
\label{sec:orgbf45fd9}
\begin{frame}[label={sec:orgd61a608}]{Recap of part 1}
...
\begin{frame}[label={sec:org99ea811}]{Recap of part 1}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgc0c3d62}]{Notation \& vocabulary}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:orgabd1f8e}]{How to set parameters: Empirical Risk Minimization}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgae71cc0}]{How to set parameters: Empirical Risk Minimization}
...
\begin{frame}[label={sec:org9a1b064}]{Evaluating a model}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org5dcfc31}]{Training error is a biased estimator of the risk}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgcba5773}]{Training error is a biased estimator of the risk}
\begin{itemize}
\item 30 different models (eg 30 possible values for \(\hat{\beta}_0, \hat{\bbeta}\))
\item All have a risk (expected accuracy) of 0.5
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org2f09e92}]{Training error is a biased estimator of the risk}
...
\begin{frame}[label={sec:orgde70e3a}]{Training error is a biased estimator of the risk}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgcf65b41}]{Training error is a biased estimator of the risk}
...
\begin{frame}[label={sec:org558be69}]{Training error is a biased estimator of the risk}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org9a53ce4}]{Training error is a biased estimator of the risk}
\begin{itemize}
...
\begin{frame}[label={sec:orgd86b198}]{Estimating prediction performance}
...
\label{sec:org6bd66b2}
\begin{frame}[label={sec:orgc4fd3b4},fragile]{scikit-learn "estimator API": \texttt{fit; predict}}
 \begin{minted}[]{python}
estimator = Ridge()
...
\begin{frame}[label={sec:org561d53c},fragile]{Evaluating performance with \texttt{sklearn.metrics}}
 \begin{minted}[]{python}
estimator = Ridge()
estimator.fit(X_train, y_train)
predictions = estimator.predict(X_test)
...
\begin{frame}[label={sec:orgc067109},fragile]{Some possible metrics for regression}
...
\subsection{cv}
\label{sec:org08fa228}
\begin{frame}[label={sec:orgd31b08c},fragile]{Cross-validation}
 \begin{center}
...
\label{sec:org327293c}
\subsection{nested cv}
\label{sec:org8c81afd}
\begin{frame}[label={sec:orge946c20}]{Need for regularization}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:orgb496712}]{Regularization}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org9b8b727}]{}
\begin{columns}
...
\begin{frame}[label={sec:org2538502}]{Setting hyperparameters}
...
\end{frame}
\begin{frame}[label={sec:orgd1aed35}]{Nested cross-validation}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgdeb67b5}]{One split}
\begin{center}
\includegraphics[width=.9\linewidth]{figures/generated/train_eval_test/datasets.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:org80e8bfd},fragile]{Nested cross-validation}
 \begin{center}
...
\begin{frame}[label={sec:orgee3f3f6},fragile]{Nested cross-validation with scikit-learn}
 \begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:org272eeff},fragile]{Implementing nested CV}
...
\label{sec:org4e61608}
\subsection{Intro}
\label{sec:org111572e}
\begin{frame}[label={sec:org133aa10}]{Dimensionality reduction}
Linear regression: projection on the column space of \(X\)
\vspace{10pt}
\begin{structureenv} %% top
\begin{columns}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org6103923}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org4b5ed94},fragile]{Simulated data for linear regression}
 \begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orge2a5284}]{Model complexity: overfitting}
\begin{itemize}
...
\label{sec:orga48b624}
\begin{frame}[label={sec:orgc06701f}]{Univariate feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:orgd742eb8}]{Original regression problem}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org212ece8}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:org2cf8b64}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:org295bf53}]{Univariate feature selection}
...
\label{sec:org6ab1b27}
\begin{frame}[label={sec:org61ee407}]{Dataset transformations}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orga6efd4d},fragile]{scikit-learn "transformer API": \texttt{fit; transform}}
 \begin{minted}[]{python}
...
\begin{frame}[label={sec:orgdcf7d2a},fragile]{\texttt{feature\_selection.SelectKBest}}
...
\begin{frame}[label={sec:orgc92371b},fragile]{Fit the transformer only on train data!}
 \begin{minted}[]{python}
transformer = SelectKBest()
transformed_train = transformer.fit_transform(X_train)
...
\begin{frame}[label={sec:org865aadb},fragile]{Pipelines}
...
\label{sec:org2a59e11}
\begin{frame}[label={sec:orgcfcd486}]{Linear decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org0ad2716}]{Linear regression: projection on the column space of \(X\)}
\begin{structureenv} %% top
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org5fce97a}]{Linear decomposition: low-rank approximation of \(\X\)}
Minimize
\begin{equation}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgb65439b}]{Linear regression after dimensionality reduction}
\begin{equation}
\hat{\y} = \W \, \hat{\bbeta}
\end{equation}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgc23cf84}]{Prediction for a new data point \(\x \in \R^{p}\)}
\begin{itemize}
...
\x \in \R^p \rightarrow \text{projection} \rightarrow \mathbold{w} \in \R^k \rightarrow \langle \cdot \, , \, \hat{\bbeta}\rangle \rightarrow \hat{y} \in \R
    \end{equation}
\end{frame}
\begin{frame}[label={sec:orgbc9a373}]{Principal Component Analysis}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orge35cb7d}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org3174a69}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orgad0b995}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orgf5038da}]{Other decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org06a3dbe}]{Ridge regression and PCA}
\begin{itemize}
...
\label{sec:orgcf10dd3}
\begin{frame}[label={sec:org0a7efd8}]{(Cross-)validation experiments are simulations}
...
\end{frame}
\begin{frame}[label={sec:orgb9f786f}]{(Cross-)validation experiments are simulations}
The validation experiments must simulate what will happen when deploying the trained model in production -- when starting to use it in real life.
...
\end{example}
\end{frame}
\begin{frame}[label={sec:orge8a6073}]{Split choice example: time series}
...
\end{frame}
\begin{frame}[label={sec:orgced9514}]{Remember that CV training sets overlap}
\begin{center}
...
\begin{frame}[label={sec:orgcdc6b7d},fragile]{Some pitfalls with cross-validation}
 \small
