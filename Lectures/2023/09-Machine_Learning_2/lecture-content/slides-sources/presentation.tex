...
\begin{frame}[label={sec:org912b016}]{Recap of part 1}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgd58bf3e}]{Notation \& vocabulary}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org28112f3}]{How to set parameters: Empirical Risk Minimization}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org388d4b1}]{How to set parameters: Empirical Risk Minimization}
...
\begin{frame}[label={sec:org8cbe96f}]{Estimating prediction performance}
...
\label{sec:orga8d9143}
\begin{frame}[label={sec:org0f2b14a},fragile]{scikit-learn "estimator API": \texttt{fit; predict}}
 \begin{minted}[]{python}
estimator = Ridge()
...
\end{frame}
\begin{frame}[label={sec:org81636f6},fragile]{Evaluating performance with \texttt{sklearn.metrics}}
 \begin{minted}[]{python}
estimator = Ridge()
estimator.fit(X_train, y_train)
predictions = estimator.predict(X_test)
...
\vfill
\href{https://github.com/neurodatascience/QLS-course-materials/blob/main/Lectures/09-Machine\_Learning\_2/exercises/ex\_01\_fit\_predict\_questions.py}{\texttt{ex\_01\_fit\_predict\_questions.py}}
\end{frame}

\begin{frame}[label={sec:org4021e1a},fragile]{Some possible metrics for regression}
...
\subsection{cv}
\label{sec:orgacb0d15}
\begin{frame}[label={sec:org68d63b2},fragile]{Cross-validation}
 \begin{center}
...
\href{https://github.com/neurodatascience/QLS-course-materials/blob/main/Lectures/09-Machine\_Learning\_2/exercises/ex\_02\_cross\_validate\_questions.py}{\texttt{ex\_02\_cross\_validate\_questions.py}}
\end{frame}
\section{Model and hyperparameter selection}
\label{sec:orgb684824}
\subsection{nested cv}
\label{sec:orga547057}
\begin{frame}[label={sec:orgbd5477a}]{Need for regularization}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org5166a89}]{Regularization}
...
\end{example}
\end{frame}
\begin{frame}[label={sec:org77598b5}]{}
\begin{columns}
...
\begin{frame}[label={sec:orge1d46f5}]{Setting hyperparameters}
...
\end{frame}
\begin{frame}[label={sec:org5358b39}]{Nested cross-validation}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgf0c62b1}]{One split}
\begin{center}
\includegraphics[width=.9\linewidth]{figures/generated/train_eval_test/datasets.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:orgd32c483},fragile]{Nested cross-validation}
 \begin{center}
...
\begin{frame}[label={sec:org27829cf},fragile]{Nested cross-validation with scikit-learn}
 \begin{itemize}
...
\href{https://github.com/neurodatascience/QLS-course-materials/blob/main/Lectures/09-Machine\_Learning\_2/exercises/ex\_03\_grid\_search\_regression\_questions.py}{\texttt{ex\_03\_grid\_search\_regression\_questions.py}}
\end{frame}
\begin{frame}[label={sec:orga3efb51},fragile]{Implementing nested CV}
 \href{https://github.com/neurodatascience/QLS-course-materials/blob/main/Lectures/09-Machine\_Learning\_2/exercises/ex\_04\_nested\_cross\_validation\_questions.py}{\texttt{ex\_04\_nested\_cross\_validation\_questions.py}}
\end{frame}
\section{Dimensionality reduction}
\label{sec:org8ab25e2}
\subsection{Intro}
\label{sec:org195c0c6}
\begin{frame}[label={sec:org2de6d35}]{Dimensionality reduction}
Linear regression: projection on the column space of \(X\)
\vspace{10pt}
\begin{structureenv} %% top
\begin{columns}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org918de0f}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org89d81f0},fragile]{Simulated data for linear regression}
 \begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org166a13a}]{Model complexity: overfitting}
\begin{itemize}
...
\label{sec:org41a4962}
\begin{frame}[label={sec:org23834bb}]{Univariate feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:orgcd36445}]{Original regression problem}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org0706725}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:orge54c497}]{After univariate feature selection}
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
\hat{\y} = \X \, \hat{\bbeta}
...
\begin{frame}[label={sec:org7a0c98d}]{Univariate feature selection}
...
\label{sec:orgb6b12c1}
\begin{frame}[label={sec:orge08cec2}]{Dataset transformations}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org8c6c2a4},fragile]{scikit-learn "transformer API": \texttt{fit; transform}}
 \begin{minted}[]{python}
...
\begin{frame}[label={sec:org0cbdb6d},fragile]{\texttt{feature\_selection.SelectKBest}}
...
\begin{frame}[label={sec:org5454054},fragile]{Fit the transformer only on train data!}
 \begin{minted}[]{python}
transformer = SelectKBest()
transformed_train = transformer.fit_transform(
    X_train, y_train)
...
\begin{frame}[label={sec:org04921a8},fragile]{Pipelines}
...
\begin{structureenv} %% links
\vfill
\href{https://github.com/neurodatascience/QLS-course-materials/blob/main/Lectures/09-Machine\_Learning\_2/exercises/ex\_05\_feature\_selection\_questions.py}{\texttt{ex\_05\_feature\_selection\_questions.py}}
\end{structureenv}
\end{frame}
\subsection{Linear decomposition methods}
\label{sec:org5d472fe}
\begin{frame}[label={sec:orge674197}]{Linear decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgb4ff2c9}]{Linear regression: projection on the column space of \(X\)}
\begin{structureenv} %% top
\begin{columns}
\begin{column}{.3\columnwidth}
\begin{equation}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org848640a}]{Linear decomposition: low-rank approximation of \(\X\)}
Minimize
\begin{equation}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org21e8914}]{Linear regression after dimensionality reduction}
\begin{equation}
\hat{\y} = \W \, \hat{\bbeta}
\end{equation}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org9b331bf}]{Prediction for a new data point \(\x \in \R^{p}\)}
\begin{itemize}
...
\x \in \R^p \rightarrow \text{projection} \rightarrow \mathbold{w} \in \R^k \rightarrow \langle \cdot \, , \, \hat{\bbeta}\rangle \rightarrow \hat{y} \in \R
    \end{equation}
\end{frame}
\begin{frame}[label={sec:org8688465}]{Principal Component Analysis}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org4f79983}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orgacf9a65}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org362d085}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orgecb286e}]{Other decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org82c534f}]{Ridge regression and PCA}
\begin{itemize}
...
\label{sec:org956e186}
\begin{frame}[label={sec:org402e3ff}]{(Cross-)validation experiments are simulations}
...
\end{frame}
\begin{frame}[label={sec:orgb51a964}]{(Cross-)validation experiments are simulations}
The validation experiments must simulate what will happen when deploying the trained model in production -- when starting to use it in real life.
...
\end{example}
\end{frame}
\begin{frame}[label={sec:orgc5f4fe5}]{Split choice example: time series}
...
\end{frame}
\begin{frame}[label={sec:orgb4dc418}]{Remember that CV training sets overlap}
\begin{center}
...
\begin{frame}[label={sec:org79fbd0e},fragile]{Some pitfalls with cross-validation}
 \small
