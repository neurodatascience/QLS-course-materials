...
def run(random_state, dimensions, dim_reduction=None):
    ...
    for n_features in dimensions:
        if dim_reduction is not None:
            ...
            model = ridge
        train_s, test_s, t = score(n_features, model, X_train, X_test, y_train, y_test)
...
n_runs = 30
all_train_scores, all_test_scores, all_durations = zip(*(run(i, dimensions) for i in range(n_runs)))
...
ax.set_title(f"Fitting a linear regression on 100 points" f"\n(median across {n_runs} simulations)")
...
ax.set_yscale("log")
fig.savefig(FIGURES_DIR / "mse_with_dim_reduction_log.pdf", bbox_inches="tight")
