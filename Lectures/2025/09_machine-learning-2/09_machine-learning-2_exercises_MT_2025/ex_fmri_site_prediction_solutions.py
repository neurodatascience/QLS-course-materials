...
if __name__ == "__main__":
    ...
    results_without_pca = cross_validate(model_without_pca, X, y, return_train_score=True)
    results_with_pca = cross_validate(model_with_pca, X, y, return_train_score=True)
    ...
    # find cluster labels
    kmeans = KMeans(init="k-means++", n_clusters=n_unique_labels, n_init=5)
...
# Answer: The optimal number of clusters is not clear for any of the scores. This is because the data is complex and clusters may correspond to different factors, not necessarily the fMRI sites.
